...
export default function DashboardPage() {
  ...
  const [summary, setSummary] = useState({ total: 0, hot: 0, readyToContact: 0 })
  ...
  const handleLeadUpdated = (updatedLead) => {
    setLeads((currentLeads) => {
      ...
      const hot = nextLeads.filter((lead) => lead.status === 'hot').length
      const readyToContact = nextLeads.filter(
        (lead) => lead.status === 'hot' || lead.status === 'warm',
      ).length
    ...
    <main className="mx-auto min-h-screen w-full max-w-6xl px-4 py-8 sm:px-8">
      <header className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <h1 className="text-2xl font-semibold text-slate-900 sm:text-3xl">Lead Dashboard</h1>
      ...
      <section className="mt-6">
        ...
        {!isLoading && !error && <LeadsTable leads={leads} onSelectLead={setSelectedLead} />}
      </section>
